  ...
  return value.map((raw) => {
    ...
    const article: ArticleInput = {
      ...
      block_rects: raw.block_rects.map((rect) => {
        ...
        return {
          block_index: Number(rect.block_index),
          x_min: Number(rect.x_min),
          y_min: Number(rect.y_min),
          x_max: Number(rect.x_max),
          y_max: Number(rect.y_max)
        };
      })
    };
...
async function status() {
  const { data: jobs, error: jobsError } = await supabaseAdmin
    ...
    .select('id,status,article_count,is_canary')
    .eq('is_canary', true);
...
async function runOne() {
  const { data: jobs, error: jobsError } = await supabaseAdmin
    ...
    .select('id,status,article_count,is_canary')
    .eq('is_canary', true)
    .in('status', ['needs_review', 'queued', 'completed'])
  ...
  for (const job of jobs || []) {
    ...
    if (!downloaded.data) throw new Error('OCR segment Google v14 source download returned no data.');
...
export async function POST(req: NextRequest) {
  try {
    ...
    return Response.json({ ok: true, result: await runOne(), status: await status() });
  } catch (error) {
