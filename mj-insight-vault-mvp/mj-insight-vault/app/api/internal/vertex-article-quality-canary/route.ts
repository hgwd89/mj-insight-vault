  ...
  properties: {
    articles: {
      ...
      items: {
        type: 'OBJECT',
        properties: {
          headline: { type: 'STRING' }, subheadline: { type: 'STRING' }, article_date: { type: 'STRING' },
          article_type: { type: 'STRING', enum: ['article','table','chart','caption','unknown'] },
          body_reconstructed: { type: 'STRING' }, facts: { type: 'ARRAY', items: { type: 'STRING' } },
          companies: { type: 'ARRAY', items: { type: 'STRING' } }, services: { type: 'ARRAY', items: { type: 'STRING' } },
          people: { type: 'ARRAY', items: { type: 'STRING' } }, confidence: { type: 'STRING', enum: ['high','medium','low'] },
          has_table: { type: 'BOOLEAN' }, has_chart: { type: 'BOOLEAN' }, has_image: { type: 'BOOLEAN' }
        },
        required: ['headline','subheadline','article_date','article_type','body_reconstructed','facts','companies','services','people','confidence','has_table','has_chart','has_image']
      }
    }
  }, required: ['articles']
};
...
export async function GET() {
  try {
    ...
    const source = Array.isArray(sourceJson) ? sourceJson[0] as Record<string, unknown> : undefined;
    ...
    const page = Array.isArray(pageJson) ? pageJson[0] as Record<string, unknown> : undefined;
    ...
    const credentials = JSON.parse(process.env.GOOGLE_CLOUD_CREDENTIALS || '') as { project_id?: string; client_email?: string; private_key?: string; token_uri?: string };
    ...
    const prompt = [
      'あなたは日経MJ紙面をニュースDB化する編集者です。画像を主情報、OCRを補助情報として使ってください。',
      '視覚的に別見出し・別記事枠がある場合だけ複数記事に分け、同じ記事を細切れにしないでください。',
      '見出し、サブ見出し、掲載日、企業名、サービス名、人物名、数字、図表、発言を忠実に拾ってください。',
      '本文は紙面の読み順を推定して自然な順序に再構成してください。主記事は最低600字、可能なら1000〜1800字程度。',
      '記事にない解釈・マーケティング示唆・提案は入れないでください。広告や購読案内は記事に混ぜないでください。',
      'article_dateは可能な場合YYYY-MM-DD。読めない場合は空文字。不確かな内容を推測で作らないでください。',
      '', 'Google Vision OCR（補助）:', ocr.slice(0, 12000)
    ].join('\n');
    ...
    const vertexRes = await fetch(endpoint, {
      ...
      body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }, { inlineData: { mimeType, data: image.toString('base64') } }] }], generationConfig: { temperature: 0.1, maxOutputTokens: 8192, responseMimeType: 'application/json', responseSchema } })
    });
