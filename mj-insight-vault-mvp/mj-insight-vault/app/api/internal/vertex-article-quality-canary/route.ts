...
async function cloudPlatformToken(credentials: {
  ...
  const response = await fetch(tokenUri, {
    ...
    body: new URLSearchParams({
      grant_type: 'urn:ietf:params:oauth:token-type:jwt-bearer'.replace('token-type', 'grant-type'),
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!sourceFileId) {
      return Response.json({ ok: true, pending: 0, message: '整理待ちはありません。' });
    }
    ...
    const source = Array.isArray(sourceJson)
      ? (sourceJson[0] as Record<string, unknown> | undefined)
      : undefined;
    ...
    const page = Array.isArray(pageJson)
      ? (pageJson[0] as Record<string, unknown> | undefined)
      : undefined;
    ...
    const token = await cloudPlatformToken(credentials as {
      client_email: string;
      private_key: string;
      token_uri?: string;
    });
    ...
    const prompt = [
      ...
      '見出し、掲載日、企業・サービス・人物・数字を忠実に保持してください。記事にない解釈や示唆を追加しないでください。',
      '本文は検索・再分析に必要な事実を落とさず自然な順序に再構成してください。原文の情報量を超えて水増ししないでください。',
    ...
    if (!vertexRes.ok) {
      return Response.json(
        { ok: false, status: vertexRes.status, error: vertexText.slice(0, 2500) },
        { status: 502 }
      );
    }
    ...
    const output =
      vertexJson.candidates?.[0]?.content?.parts?.map((p) => p.text || '').join('').trim() || '';
    ...
    return Response.json(
      {
        ...
        articles: articles.map((a) => ({
          ...
          body_preview:
            typeof a.body_reconstructed === 'string' ? a.body_reconstructed.slice(0, 500) : '',
