...
async function cloudPlatformToken(credentials: { client_email: string; private_key: string; token_uri?: string }) {
  ...
  if (!response.ok) {
    // Retry with the standard grant string; keeping this branch canary self-contained.
    ...
    if (!retry.ok) throw new Error(`Google OAuth ${retry.status}: ${retryText.slice(0, 500)}`);
    const retryJson = JSON.parse(retryText) as { access_token?: string };
    if (!retryJson.access_token) throw new Error('Google OAuth token missing');
...
export async function GET() {
  try {
    ...
    const credentials = JSON.parse(raw) as { project_id?: string; client_email?: string; private_key?: string; token_uri?: string };
    ...
    return Response.json({
      ...
      articles: articles.map((a) => ({
        ...
        facts_count: Array.isArray(a.facts) ? a.facts.length : 0
      })),
