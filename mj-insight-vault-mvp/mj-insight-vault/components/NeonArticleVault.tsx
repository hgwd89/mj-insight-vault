  ...
  return (
    <div className="space-y-4">
      <section className="card p-5">
        <p className="text-sm font-bold text-emerald-700">記事一覧</p>
        <h1 className="mt-1 text-xl font-black sm:text-2xl">OCRから整理した記事を読む</h1>
        <p className="mt-2 text-sm leading-6 text-zinc-600">
          原本画像ではなく、紙面ごとにOCR・記事分割・本文再構成した記事を表示します。
        </p>
      ...
      <section className="space-y-3">
        {!loading && rows.length === 0 ? (
          <div className="card p-6 text-center">
            <p className="font-bold text-zinc-800">整理済みの記事はまだありません。</p>
            <p className="mt-2 text-sm leading-6 text-zinc-500">「資料追加」でOCR・記事整理を実行すると、ここに記事単位で表示されます。</p>
          ...
          >
            ...
            <div className="mt-3 text-right text-xs font-bold text-zinc-700">記事を読む →</div>
          </Link>
        ))}
      </section>
    </div>
