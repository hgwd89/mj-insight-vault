...
function moveCoverageToEnd(value: string) {
  ...
  body = body.replace(/根拠確認用記事数\s*[:：]\s*(\d+)件/g, '根拠記事リスト: $1件');
  body = body.replaceAll('直接該当率ではありません', '全件カバレッジとは別の入力制御です');

  const coverageMarker = '## 0.0 システムカバレッジ';
  const conclusionMarkers = ['## 1. 結論', '## 1 結論', '## 結論', '# 結論'];
  const coverageIndex = body.indexOf(coverageMarker);
  if (coverageIndex !== 0) return body.trim();

  let conclusionIndex = -1;
  for (const marker of conclusionMarkers) {
    const index = body.indexOf(marker);
    if (index > 0 && (conclusionIndex < 0 || index < conclusionIndex)) conclusionIndex = index;
  }
  if (conclusionIndex < 0) return body.trim();
...
function refutationFallback(answer: JsonRecord) {
  const claims = [...asArray(answer.major_trends), ...asArray(answer.explanatory_hypotheses), ...asArray(answer.cross_article_insights)].filter(isRecord).slice(0, 5);
  if (!claims.length) return [{ target_claim: '主要主張全体', possible_counterargument: '記事群が企業施策・商品投入中心で、生活者側の動機が直接示されていない可能性がある。', evidence_gap: '生活者発話、購買継続、比較対象、反例データが不足している可能性。', downgrade_or_revision: '断定ではなく、調査で検証すべき仮説として扱う。', falsification_condition: '追加調査で該当行動が一部カテゴリや一時的話題に限定されると確認された場合。' }];
  return claims.map((claim, index) => ({ target_claim: text(claim.claim || claim.trend || claim.hypothesis || claim.title || `主要主張 ${index + 1}`), possible_counterargument: text(claim.alternative_read) || '企業施策やカテゴリ事情を、生活者変化として読み替えすぎている可能性がある。', evidence_gap: '記事根拠だけでは、行動の持続性・広がり・心理要因までは断定できない。', downgrade_or_revision: '根拠が直接的でない部分は「仮説」「可能性」「調査が必要」として扱う。', falsification_condition: '反例記事、購買継続率、生活者発話、カテゴリ外比較で支持されない場合。' }));
}

function negativeSpaceFallback(answer: JsonRecord) {
  const coverage = isRecord(answer.source_coverage) ? answer.source_coverage : {};
  return [{ expected_but_weak_or_absent_theme: '生活者本人の発話・継続行動・不満の直接証拠', why_absence_matters: 'MJ記事は企業施策・商品投入・売場側の情報が多く、生活者心理を直接観察しているとは限らない。', what_to_check_next: '該当テーマについて、実購買者・非購買者の発話、継続率、比較対象カテゴリの反例を確認する。', coverage_context: text(coverage.coverage_note) }];
}
...
function confidenceRubricFallback(answer: JsonRecord) {
  ...
  if (!evidence.length) return [{ claim: '主要結論全体', confidence: 'C', reason_for_confidence: '記事群から方向性は読めるが、根拠マトリクスが弱い。', reason_for_uncertainty: '直接の生活者発話・反例・継続性の確認が不足。' }];
  return evidence.map((item) => ({ claim: text(item.claim || '根拠付き主張'), confidence: text(item.evidence_strength || 'B'), reason_for_confidence: text(item.what_can_be_said || item.evidence_excerpt_or_fact || '記事事実に基づく。'), reason_for_uncertainty: text(item.what_cannot_be_said || item.limitation || 'この記事単独では生活者全体の傾向とは断定できない。') }));
}

function researchNeedsFallback(answer: JsonRecord) {
  const evidence = asArray(answer.evidence_matrix).filter(isRecord).slice(0, 5);
  if (!evidence.length) return [{ question: '記事群から見える生活者変化は、実際の生活者発話・購買継続・非購買理由で支持されるか。', why_it_matters: '記事は企業施策や市場側の動きに偏りやすく、生活者インサイトとして断定するには追加検証が必要。', needed_data: '生活者インタビュー、購買・利用継続データ、反例カテゴリ、非利用者の理由。', method_hint: 'N1深掘り、投影法、生活文脈インタビュー、定量共感検証。', priority: 'high' }];
  return evidence.map((item, index) => ({ question: `「${text(item.claim || `根拠候補 ${index + 1}`)}」は生活者側の行動・心理として再現性があるか。`, why_it_matters: text(item.what_cannot_be_said || item.limitation || '記事単独では生活者全体の傾向とは断定できないため。'), needed_data: '生活者発話、購買・利用継続、非利用理由、カテゴリ横断の反例。', method_hint: '該当記事を刺激材にしたN1深掘りと、共感・行動意向の定量確認。', evidence_article_link: articleLinkFromRecord(item), priority: index < 2 ? 'high' : 'medium' }));
}

function researchNeedsMarkdown(answer: JsonRecord) {
  const needs = asArray(answer.research_needs).filter(isRecord).slice(0, 6);
  if (!needs.length) return '';
  return ['## 10.7 調査論点補修', ...needs.map((need, index) => {
...
function coverageBlock(answer: JsonRecord, result: JsonRecord, relatedArticles: unknown[]) {
  ...
  return { article_count: retrieved, full_corpus_article_count: total, monthly_rollup_source_article_count: rollupCount, scanned_article_count: scanned, final_article_count: final, scan_model: scanModel, scan_enabled: scanEnabled, retrieval_mode: mode, coverage_note: scanEnabled ? `全件カバレッジは${total}件、スキャンは${scanned}件、月別rollup対象は${rollupCount ?? '-'}件です。最終投入${final ?? '複数'}件はLLM個別本文投入用であり、全件カバレッジとは別の入力制御です。全記事本文をそのまま単一プロンプトへ丸投げする設計ではありません。` : `対象記事${retrieved}件から分析しています。`, limitation: '最終レポートは選抜記事と月別rollupに基づく統合分析です。スキャン外・選抜外の記事に反例がある可能性は、調査論点として残します。' };
}
...
function qualityAppendix(checks: QualityCheck[]) {
  ...
  return ['## 11. 品質ゲート補足', '以下の項目は、後処理バリデータで不足または弱さが検知されました。結論として断定せず、調査論点として扱ってください。', ...failed.map((check) => `- ${check.key}: ${check.note}`)].join('\n');
}

function qualityAuditText(checks: QualityCheck[]) {
  const failed = checks.filter((check) => !check.passed);
  if (!failed.length) return '';
  return ['## 12. 分析品質監査', `不足検知: ${failed.length}件`, ...failed.map((check) => `- ${check.key}: ${check.note}`)].join('\n');
}
...
export function enhanceChatAnalysisResult<T>(result: T): T {
  ...
  let body = moveCoverageToEnd(text(answer.answer_text));

  answer.coverage_diagnosis = { ...coverageBlock(answer, result, relatedArticles), ...(isRecord(answer.coverage_diagnosis) ? answer.coverage_diagnosis : {}) };
  answer.source_coverage = { ...(isRecord(answer.source_coverage) ? answer.source_coverage : {}), ...coverageBlock(answer, result, relatedArticles) };
  ...
  const systemCoverageText = [
    ...
    `LLM個別本文投入記事数: ${text(coverage.final_article_count || source.final_article_count) || '-'}件`,
    '注記: LLM個別本文投入記事数は、全件カバレッジとは別の入力制御です。全体分析の母集団は月別rollupと全件スキャンです。',
    `選抜方式: ${text(coverage.coverage_note || source.coverage_note) || '-'}`,
    ''
  ].join('\n');

  if (!body.includes('## 99. カバレッジ・システム情報')) body = `${body}\n\n${systemCoverageText}`.trim();
  ...
  if (researchNeeds && !body.includes('## 10.7 調査論点補修')) body = `${body}\n\n${researchNeeds}`.trim();
  ...
  answer.quality_gate = { status: checks.every((check) => check.passed) ? 'passed' : 'needs_review', checks, failed_checks: checks.filter((check) => !check.passed).map((check) => check.key), note: 'この品質ゲートは後処理バリデータです。独立LLMによる別人格監査ではありません。' };
