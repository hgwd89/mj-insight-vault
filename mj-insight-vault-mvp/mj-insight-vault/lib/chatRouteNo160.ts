...
function fallbackModel(primary: string) { const configured = text(process.env.OPENAI_FINAL_FALLBACK_MODEL || 'gpt-4.1-mini'); if (configured && configured !== primary && models().includes(configured)) return configured; return primary === 'gpt-4.1-mini' ? 'gpt-4o-mini' : 'gpt-4.1-mini'; }
function reasoningOptions(model: string) { return model.startsWith('gpt-5') ? { reasoning_effort: 'low' as const } : {}; }
...
function unusableReportError(label: string, completion: { choices: Array<{ finish_reason?: string | null }>; usage?: { completion_tokens?: number; completion_tokens_details?: { reasoning_tokens?: number } } | null }) {
  const finishReason = completion.choices[0]?.finish_reason || 'unknown';
  const completionTokens = completion.usage?.completion_tokens ?? 'unknown';
  const reasoningTokens = completion.usage?.completion_tokens_details?.reasoning_tokens ?? 'unknown';
  return new Error(`${label} returned empty report JSON (finish_reason=${finishReason}, completion_tokens=${completionTokens}, reasoning_tokens=${reasoningTokens})`);
}
type ApiCompletion = { choices: Array<{ finish_reason?: string | null; message: { content?: string | null } }>; usage?: { prompt_tokens?: number; completion_tokens?: number; total_tokens?: number; completion_tokens_details?: { reasoning_tokens?: number } } | null };
...
function buildEmergencyAnswer(query: string, base: Record<string, unknown>, finalArticles: WideArticle[], error: string) {
  ...
  return { ...base, report_title: '暫定レポート：AI本文生成が完了しませんでした', answer_text: ['## 結論', 'AI本文生成がタイムアウトまたは空応答になったため、暫定出力です。全記事取得と月別rollup参照、根拠候補抽出は完了していますが、深い統合分析は未完了です。', '', '## 状態', `- ユーザー指示: ${query}`, `- エラー: ${error}`, `- 取得記事数: ${text((sourceCoverage as Record<string, unknown>).article_count)}`, `- 根拠確認用記事: ${finalArticles.length}件`, '', '## 代表記事候補', articleLines || '代表記事候補なし', '', '## 根拠と限界', 'この出力はタイムアウト時の安全フォールバックです。通常レポートより浅いため、月別rollup生成状態を確認したうえで再分析してください。'].join('\n'), generation_warning: 'emergency_generation_fallback', generation_error: error };
}
...
async function runWide(body: Record<string, unknown>, onProgress?: ProgressReporter) {
  ...
  let writerDiagnostics: Record<string, unknown> = {};
  const rollupInputChars = monthlyContext?.context_text?.length || 0;
  const articleInputChars = compactInput.reduce((sum: number, a) => sum + a.text.length, 0);
  ...
  if (!openai) {
    ...
    try {
      ...
      writerDiagnostics = { model: selectedModel, elapsed_ms: Date.now() - generationStartMs, prompt_tokens: primaryCompletion?.usage?.prompt_tokens ?? null, completion_tokens: primaryCompletion?.usage?.completion_tokens ?? null, finish_reason: primaryCompletion?.choices[0]?.finish_reason ?? null, article_count: finalArticles.length, rollup_input_chars: rollupInputChars, article_input_chars: articleInputChars, result: 'primary_failed' };
      ...
      try {
        ...
        writerDiagnostics = { ...writerDiagnostics, fallback_model: fbModel, fallback_elapsed_ms: Date.now() - generationStartMs, fallback_completion_tokens: fallbackCompletion?.usage?.completion_tokens ?? null, fallback_finish_reason: fallbackCompletion?.choices[0]?.finish_reason ?? null, result: 'fallback_success' };
      } catch (fallbackError) {
        const fallbackMessage = `${fallbackError instanceof Error ? fallbackError.message : 'fallback report generation failed'}${completionDiag(fallbackCompletion)}`;
        ...
        writerDiagnostics = { ...writerDiagnostics, fallback_model: fbModel, fallback_completion_tokens: fallbackCompletion?.usage?.completion_tokens ?? null, fallback_finish_reason: fallbackCompletion?.choices[0]?.finish_reason ?? null, result: 'emergency_fallback' };
      }
    }
  }
  ...
  const shouldRunCycle =
    ...
    !generationWarning.includes('emergency_fallback_used');
  if (shouldRunCycle) {
