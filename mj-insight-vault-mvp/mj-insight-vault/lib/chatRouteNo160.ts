...
async function runWide(body: Record<string, unknown>, onProgress?: ProgressReporter) {
  ...
  if (!openai) {
    ...
    try {
      ...
      if (!hasUsableReportJson(parsed)) throw unusableReportError('model', completion);
    } catch (primaryError) {
