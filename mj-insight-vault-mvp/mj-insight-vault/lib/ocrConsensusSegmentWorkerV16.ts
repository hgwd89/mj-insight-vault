...
type BuiltArticle = {
  ...
  reading: Awaited<ReturnType<typeof buildArticleReadingSegmentsV15>>;
};
type SegmentReceipt = {
  article_id: string;
  sequence: number;
...
function errorMessage(error: unknown) {
  ...
  return text(error) || 'OCR consensus segment v16 worker failed';
}
...
function configuredModels() {
  ...
  if (!sol || !terra || sol === terra) throw new StructuralOutputError('OCR consensus segment v16 requires two distinct models.');
  return { sol, terra };
}
...
async function claimCanary(): Promise<Claim | null> {
  ...
  if (!claim.id || !claim.source_job_id || !claim.lease_token || !claim.is_canary || claim.article_count < 1) {
    throw new StructuralOutputError('OCR consensus segment v16 claim is invalid or non-canary.');
  }
  return claim;
}
...
async function loadInput(claim: Claim): Promise<LoadedInput> {
  ...
  if (error) throw error;
  if (!isRecord(data) || !isRecord(data.source) || !Array.isArray(data.articles)) throw new StructuralOutputError('OCR consensus segment v16 input payload is invalid.');
  const source = data.source;
  const storagePath = text(source.storage_path);
  const width = Number(source.width || 0), height = Number(source.height || 0);
  if (!storagePath || width < 1 || height < 1) throw new StructuralOutputError('OCR consensus segment v16 source metadata is incomplete.');

  const articles = data.articles.map((raw) => {
    if (!isRecord(raw) || !Array.isArray(raw.block_rects)) throw new StructuralOutputError('OCR consensus segment v16 article input is invalid.');
    const article: ArticleInput = {
      ...
      block_rects: raw.block_rects.map((rect) => {
        if (!isRecord(rect)) throw new StructuralOutputError('OCR consensus segment v16 block rectangle is invalid.');
    ...
    if (!article.article_id || article.crop_version !== 'article_geometry_mask_composite_v3' || !article.crop_spec_sha256 || !article.crop_image_sha256 || !article.google_text || !article.source_image_sha256 || !article.block_rects.length) {
      throw new StructuralOutputError(`OCR consensus segment v16 article binding is incomplete: ${article.article_id || 'unknown'}`);
    }
    return article;
  });
  if (articles.length !== claim.article_count || new Set(articles.map((item) => item.article_id)).size !== claim.article_count) {
    throw new StructuralOutputError('OCR consensus segment v16 article set is not bijective.');
  }

  const downloaded = await supabaseAdmin.storage.from(STORAGE_BUCKET).download(storagePath);
  if (downloaded.error) throw downloaded.error;
  if (!downloaded.data) throw new StructuralOutputError('OCR consensus segment v16 source download returned no data.');
  const image = Buffer.from(await downloaded.data.arrayBuffer());
  const sourceImageSha256 = sha256(image);
  for (const article of articles) {
    if (article.source_image_sha256 !== sourceImageSha256) throw new StructuralOutputError(`OCR consensus segment v16 source image binding changed: ${article.article_id}`);
  }
...
async function buildArticle(input: LoadedInput, article: ArticleInput): Promise<BuiltArticle> {
  ...
  if (crop.cropSpecSha256 !== article.crop_spec_sha256 || crop.cropImageSha256 !== article.crop_image_sha256) {
    throw new StructuralOutputError(`OCR consensus segment v16 crop fingerprint changed: ${article.article_id}`);
  }
  const reading = await buildArticleReadingSegmentsV15({
    articleId: article.article_id,
    compositeBuffer: crop.buffer,
    compositeWidth: crop.width,
    compositeHeight: crop.height,
    compositeImageSha256: crop.cropImageSha256
  });
  if (!reading.segments.length) throw new StructuralOutputError(`OCR consensus segment v16 produced no reading segments: ${article.article_id}`);
...
async function getSegmentReceipts(jobId: string, passKind: PassKind, articleId: string): Promise<SegmentReceipt[]> {
  const { data, error } = await supabaseAdmin.from('ocr_independent_segment_receipts_v16')
    .select('article_id,sequence,segment_count,model,segmentation_version,segmentation_spec_sha256,segment_image_sha256,transcription,confidence,proper_noun_status,visual_proper_nouns,output_contract_status,reason,provider_response_id,prompt_sha256,response_sha256')
...
function segmentResponseFormat(sequence: number) {
  return {
    type: 'json_schema', name: 'mj_independent_ocr_segment_v16', strict: true,
    schema: {
...
async function callSegmentVision(input: { model: string; passKind: PassKind; articleId: string; sequence: number; segmentCount: number; segment: BuiltArticle['reading']['segments'][number]; segmentationVersion: string; segmentationSpecSha256: string }) {
  const apiKey = getOpenAIKey();
  if (!apiKey) throw new StructuralOutputError('OPENAI_API_KEY is not configured.');
  const role = input.passKind === 'sol' ? 'primary independent OCR segment transcriber' : 'second independent OCR segment transcriber using a different model';
  const instructions = [
    `You are a ${role} for a single isolated Japanese newspaper or magazine reading segment.`,
    'You receive exactly ONE segment image. You do not receive Google OCR, candidate text, the article overview, adjacent segments, or any other segment from the article.',
    'Transcribe ONLY characters visibly present in this segment. This is verbatim transcription, not article reconstruction.',
    'Never use likely sentence context to complete text at the top, bottom, left, or right boundary. Never continue a word or sentence beyond visible pixels.',
    'Never summarize, paraphrase, normalize, rewrite, reorder, or silently repair characters, numbers, punctuation, names, dates, prices, percentages, or quantities.',
    'For every genuinely unreadable or clipped character position, emit 〓 rather than guessing. Preserve visible line breaks when they are meaningful inside the segment.',
    'For Japanese vertical text, read top-to-bottom inside the supplied segment only. Segment-to-segment order is handled later by deterministic code and is not your task.',
    'confidence is confidence in this segment transcription only. Lower it when any visible content is unreadable, clipped, overlapping, or ambiguous.',
    'visual_proper_nouns must contain only proper nouns copied verbatim from this segment transcription and visibly confirmed in this segment. If none exist, use not_applicable and an empty array. If an important visible proper noun cannot be read, use failed.',
    'Return exactly the requested structured object. Do not add prose outside it.'
  ].join('\n');
  const metadata = JSON.stringify({
    task: 'independent_visual_ocr_segment_v16', pass_kind: input.passKind, article_id: input.articleId,
    sequence: input.sequence, segment_count: input.segmentCount, segmentation_version: input.segmentationVersion,
    segmentation_spec_sha256: input.segmentationSpecSha256, segment_image_sha256: input.segment.imageSha256
  });
  const promptSha = sha256([input.model,input.passKind,instructions,metadata,input.segment.imageSha256].join('\n---\n'));
  const content: Array<Record<string, unknown>> = [
    { type: 'input_text', text: metadata },
    { type: 'input_image', image_url: `data:${input.segment.mimeType};base64,${input.segment.buffer.toString('base64')}`, detail: 'high' }
  ];
  ...
  try {
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({ model: input.model, store: false, max_output_tokens: 8000, instructions, input: [{ role: 'user', content }], text: { format: segmentResponseFormat(input.sequence) } })
    });
    const raw = await response.text();
    if (!response.ok) throw new ProviderError(`OpenAI OCR segment v16 failed: ${response.status} ${response.statusText} ${raw.slice(0, 1800)}`, response.status === 408 || response.status === 409 || response.status === 429 || response.status >= 500);
    let json: JsonRecord;
    try { json = JSON.parse(raw) as JsonRecord; } catch { throw new ProviderError('OpenAI OCR segment v16 response JSON is malformed.', true); }
    const responseId = text(json.id), output = extractResponseText(json);
    if (!responseId || !output) throw new ProviderError('OpenAI OCR segment v16 response receipt or output is missing.', true);
    let parsed: JsonRecord;
    try { parsed = JSON.parse(output) as JsonRecord; } catch { throw new StructuralOutputError('OpenAI OCR segment v16 structured output is malformed.'); }
    ...
    if (error instanceof Error && error.name === 'AbortError') throw new ProviderError('OpenAI OCR segment v16 request timed out.', true);
    if (error instanceof TypeError) throw new ProviderError(`OpenAI OCR segment v16 network failure: ${error.message}`, true);
    throw error;
  } finally { clearTimeout(timer); }
}

function sanitizeSegment(raw: JsonRecord, sequence: number) {
  if (Number(raw.sequence) !== sequence) throw new StructuralOutputError(`OCR segment v16 sequence mismatch: expected=${sequence} actual=${raw.sequence}`);
  const transcription = String(raw.transcription ?? '').trim();
  if (!transcription) throw new StructuralOutputError(`OCR segment v16 transcription is empty: sequence=${sequence}`);
  const confidence = Number(raw.confidence);
  if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1) throw new StructuralOutputError(`OCR segment v16 confidence is invalid: sequence=${sequence}`);
...
async function appendSegment(claim: Claim, passKind: PassKind, model: string, built: BuiltArticle, sequence: number) {
  const segment = built.reading.segments.find((item) => item.sequence === sequence);
  if (!segment) throw new StructuralOutputError(`OCR segment v16 missing generated segment: ${sequence}`);
  const result = await callSegmentVision({
    model, passKind, articleId: built.article.article_id, sequence, segmentCount: built.reading.segments.length,
    segment, segmentationVersion: built.reading.version, segmentationSpecSha256: built.reading.segmentationSpecSha256
  });
  const row = sanitizeSegment(result.parsed, sequence);
  const { data, error } = await supabaseAdmin.rpc('append_ocr_independent_segment_v16', {
    p_job_id: claim.id, p_lease_token: claim.lease_token, p_pass_kind: passKind, p_article_id: built.article.article_id,
    p_sequence: sequence, p_segment_count: built.reading.segments.length, p_model: model,
    p_segmentation_version: built.reading.version, p_segmentation_spec_sha256: built.reading.segmentationSpecSha256, p_segment_image_sha256: segment.imageSha256,
    p_transcription: row.transcription, p_confidence: row.confidence, p_proper_noun_status: row.properNounStatus,
    p_visual_proper_nouns: row.nouns, p_output_contract_status: row.outputContractStatus, p_reason: row.reason,
...
async function assembleArticle(claim: Claim, passKind: PassKind, model: string, built: BuiltArticle, receipts: SegmentReceipt[]) {
  const expectedCount = built.reading.segments.length;
  if (receipts.length !== expectedCount) throw new StructuralOutputError(`OCR segment v16 assembly incomplete: ${receipts.length}/${expectedCount}`);
  for (let i = 0; i < expectedCount; i += 1) {
    const expected = built.reading.segments[i];
    const receipt = receipts[i];
    if (receipt.sequence !== expected.sequence || receipt.segment_count !== expectedCount || receipt.model !== model
      || receipt.segmentation_version !== built.reading.version || receipt.segmentation_spec_sha256 !== built.reading.segmentationSpecSha256
      || receipt.segment_image_sha256 !== expected.imageSha256) {
      throw new StructuralOutputError(`OCR segment v16 receipt binding mismatch: article=${built.article.article_id} sequence=${expected.sequence}`);
    }
  }
  const transcription = receipts.map((row) => row.transcription).join('\n');
  const confidence = Math.min(...receipts.map((row) => row.confidence));
  const nounSet = new Set(receipts.flatMap((row) => row.visual_proper_nouns));
  const nouns = [...nounSet];
  ...
  const reason = `mechanical_segment_concatenation_v16 segments=${expectedCount} min_segment_confidence=${confidence.toFixed(4)} contract=${outputContractStatus}`;
  const providerResponseId = `segment-bundle-v16:${sha256(receipts.map((row) => row.provider_response_id).join('|'))}`;
  ...
  const { data, error } = await supabaseAdmin.rpc('append_ocr_independent_pass_v11', {
    ...
    p_provider_response_id: providerResponseId, p_prompt_sha256: promptSha, p_response_sha256: responseSha,
    p_input_binding_sha256: inputBinding(built.article),
...
async function runPassStep(claim: Claim, input: LoadedInput, passKind: PassKind, model: string, allowedIds?: Set<string>) {
  ...
  for (const article of input.articles) {
    ...
    const receipts = await getSegmentReceipts(claim.id, passKind, article.article_id);
    const existingSequences = new Set(receipts.map((row) => row.sequence));
    const missing = built.reading.segments.find((segment) => !existingSequences.has(segment.sequence));
    if (missing) {
      return { action: 'segment', article_id: article.article_id, sequence: missing.sequence, segment_count: built.reading.segments.length, result: await appendSegment(claim, passKind, model, built, missing.sequence), external_calls: 1 };
    }
    return { action: 'assemble', article_id: article.article_id, segment_count: built.reading.segments.length, result: await assembleArticle(claim, passKind, model, built, receipts), external_calls: 0 };
  }
...
async function evaluateUndecided(claim: Claim, input: LoadedInput) {
  ...
  for (const article of input.articles) {
    ...
    else if (!['passed_single','passed_two_model','needs_review','sol_required'].includes(status)) throw new StructuralOutputError(`OCR consensus segment v16 decision state is invalid: ${status || 'empty'}`);
  }
...
export async function getOcrConsensusSegmentV16Status() {
  ...
  let segmentReceipts = 0;
  let articleTranscriptions = 0;
  if (ids.length) {
    const segmentResult = await supabaseAdmin.from('ocr_independent_segment_receipts_v16').select('id', { count: 'exact', head: true }).in('job_id', ids);
    if (segmentResult.error) throw segmentResult.error;
    segmentReceipts = segmentResult.count || 0;
  ...
  return { canary_jobs: (jobs || []).length, jobs, segment_receipts: segmentReceipts, article_transcriptions: articleTranscriptions };
}

export async function runOcrConsensusSegmentV16Step() {
  ...
  try {
    ...
    if (terraRequired.size) {
      ...
      if (terraRequired.size) throw new StructuralOutputError(`OCR consensus segment v16 still requires Terra after completion: ${[...terraRequired].join(',')}`);
    }

    const decided = await existingDecisionIds(claim.id);
    if (decided.size !== claim.article_count) throw new StructuralOutputError(`OCR consensus segment v16 decisions incomplete: ${decided.size}/${claim.article_count}`);
