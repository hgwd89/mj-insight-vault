...
(async () => {
  ...
  const { data: claim, error: claimError } = await supabaseAdmin.rpc('claim_next_inventory_build_drain_ticket_v1', {
  ...
  const ticketKey = String(claim.ticket_key || '');
  if (!ticketKey) throw new Error('inventory_build_ticket_key_missing');
  ...
  try {
    ...
    result = { ticket_key: ticketKey, workers, active_ms: activeMs, stop_reason: stopReason, fatal_error: fatalError };
  }
