...
assert(currentPiece.includes('assertNoLegacyCanaryPieceReceipts()'), 'Current piece route must retain legacy-piece preflight blocking.');
assert(currentPiece.includes('ARTICLE_BLOCK_READING_VERSION_V21'), 'Current piece route must remain bound to V21 reading pieces.');
assert(currentPiece.includes('Promise.all(['), 'Current piece route must retain bounded parallel workers.');
...
console.log('verify-retired-ocr-execution-routes-v24: ok (legacy OCR POST execution fail-closed; current piece route remains active)');
