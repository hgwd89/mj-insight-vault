  ...
  select exists (
    ...
    where r.id = p_run_id
      ...
      and not exists (
        ...
        where b.run_id = r.id
          and (
            ...
            or array(select article_id::text from unnest(b.article_ids) article_id order by article_id::text) <> array(
              select value from jsonb_array_elements_text(case when jsonb_typeof(b.summary_json -> 'read_article_ids') = 'array' then b.summary_json -> 'read_article_ids' else '[]'::jsonb end) value order by value
            )
      ...
      and not exists (
        ...
        where b.run_id = r.id and a.id is null
      )
...
as $$
  with evidence_items as (
    select item from jsonb_array_elements(case when jsonb_typeof(p_payload -> 'evidence_matrix') = 'array' then p_payload -> 'evidence_matrix' else '[]'::jsonb end) item
  ),
  valid_evidence as (
    ...
    where jsonb_typeof(item) = 'object'
      ...
      and exists (select 1 from public.formal_corpus_articles_v1 a where a.id = coalesce(item ->> 'article_id', item ->> 'id')::uuid)
      and exists (select 1 from public.full_corpus_scan_batches b where b.run_id = p_run_id and coalesce(item ->> 'article_id', item ->> 'id')::uuid = any(b.article_ids))
  )
  select
    (select count(distinct article_id) from valid_evidence) >= 3
    and exists (select 1 from jsonb_array_elements(case when jsonb_typeof(p_payload -> 'refutation_audit') = 'array' then p_payload -> 'refutation_audit' else '[]'::jsonb end) item where jsonb_typeof(item) = 'object' and lower(coalesce(item ->> 'synthetic_repair', 'false')) not in ('true', '1', 'yes'))
    and exists (select 1 from jsonb_array_elements(case when jsonb_typeof(p_payload -> 'research_needs') = 'array' then p_payload -> 'research_needs' else '[]'::jsonb end) item where jsonb_typeof(item) = 'object' and lower(coalesce(item ->> 'synthetic_repair', 'false')) not in ('true', '1', 'yes'))
    and exists (select 1 from jsonb_array_elements(case when jsonb_typeof(p_payload -> 'negative_space') = 'array' then p_payload -> 'negative_space' else '[]'::jsonb end) item where jsonb_typeof(item) = 'object' and lower(coalesce(item ->> 'synthetic_repair', 'false')) not in ('true', '1', 'yes'))
    and exists (select 1 from jsonb_array_elements(case when jsonb_typeof(p_payload -> 'confidence_rubric') = 'array' then p_payload -> 'confidence_rubric' else '[]'::jsonb end) item where jsonb_typeof(item) = 'object' and lower(coalesce(item ->> 'synthetic_repair', 'false')) not in ('true', '1', 'yes'));
$$;
...
begin
  ...
  formal := full_corpus_candidate and metadata_integrity and database_run_integrity and database_raw_integrity and new.source_job_id is not null and gate_version = 'formal_gate_v2' and validation_mode = 'raw_before_enrichment' and quality_status = 'passed' and not provisional and not fallback_used;

  if tg_op = 'INSERT' and full_corpus_candidate and gate_version <> 'formal_gate_v2' then raise exception using errcode = '23514', message = 'formal_report_gate_version_missing', detail = 'A full-corpus report must pass formal_gate_v2 before persistence.'; end if;
  if tg_op = 'INSERT' and full_corpus_candidate and not metadata_integrity then raise exception using errcode = '23514', message = 'formal_report_integrity_gate_missing', detail = 'A formal report requires full_corpus_batch_v2 and an all-batch final context with zero omissions.'; end if;
  if tg_op = 'INSERT' and full_corpus_candidate and not database_run_integrity then raise exception using errcode = '23514', message = 'formal_report_run_integrity_failed', detail = 'The referenced scan run does not pass database-recomputed v2 batch integrity.'; end if;
  if tg_op = 'INSERT' and full_corpus_candidate and not database_raw_integrity then raise exception using errcode = '23514', message = 'formal_report_raw_evidence_integrity_failed', detail = 'The raw report evidence does not pass database-recomputed evidence integrity.'; end if;
  if tg_op = 'INSERT' and full_corpus_candidate and new.source_job_id is null then raise exception using errcode = '23514', message = 'formal_report_source_job_missing', detail = 'Formal reports must be linked to a durable report job.'; end if;
  ...
  new.analysis_verification_status := case when formal then 'full_corpus_verified' when new.report_kind = 'followup' then 'derived_followup' when new.report_kind = 'diagnostic' then 'blocked_diagnostic' when not metadata_integrity and full_corpus_candidate then 'integrity_unverified' when not database_run_integrity and full_corpus_candidate then 'run_integrity_unverified' when not database_raw_integrity and full_corpus_candidate then 'raw_evidence_unverified' when new.source_job_id is null and full_corpus_candidate then 'source_job_unverified' when provisional then 'provisional_unverified' when fallback_used then 'fallback_unverified' when gate_version <> 'formal_gate_v2' then 'gate_version_unverified' else 'quality_unverified' end;
  return new;
end;
$$;

update public.chat_reports set answer_json = answer_json where full_corpus_gate = 'passed' or is_formal_report = true;