-- Run resumable report jobs independently of an open browser tab.
-- Operational prerequisite: Supabase Vault must contain a secret named
-- `mj_report_worker_password` with the Vercel APP_PASSWORD value.
...
begin
  select jobid
    into v_job_id
  from cron.job
  where jobname = 'mj_report_worker'
  limit 1;
