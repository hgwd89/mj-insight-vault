...
begin
  ...
  v_slots := greatest(0, least(3, v_queued + v_retryable) - v_running);
  ...
  return jsonb_build_object(
    'status', case when v_sent > 0 then 'kicked' else 'idle' end,
    'queued', v_queued,
    'running', v_running,
...
revoke all on function public.kick_article_classification_loop_v1() from public, anon, authenticated;
grant execute on function public.kick_article_classification_loop_v1() to service_role;
