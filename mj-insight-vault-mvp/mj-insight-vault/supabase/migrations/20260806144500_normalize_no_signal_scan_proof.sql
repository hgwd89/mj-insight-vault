-- Normalize legitimate evidence-bearing no-signal batches so the application and
-- database integrity gates use the same auditable contract.
...
begin
  ...
  v_validated := lower(coalesce(new.summary_json ->> 'analysis_is_validated', 'false')) in ('true', '1', 'yes')
    and lower(coalesce(new.summary_json ->> 'fallback_used', 'false')) not in ('true', '1', 'yes');
  v_has_evidence := jsonb_array_length(
    case when jsonb_typeof(new.summary_json -> 'evidence') = 'array'
      then new.summary_json -> 'evidence' else '[]'::jsonb end
  ) > 0;
  v_has_signal :=
  ...
  v_failures := case
    when jsonb_typeof(new.summary_json #> '{validation,failures}') = 'array'
      then new.summary_json #> '{validation,failures}'
    else '[]'::jsonb
  end;
  ...
  select coalesce(jsonb_agg(article_id::text order by article_id::text), '[]'::jsonb)
    into v_allowed_ids
  from unnest(coalesce(new.article_ids, '{}'::uuid[])) article_id;

  v_reported_read_ids := case
  ...
  new.summary_json := jsonb_set(
    ...
    jsonb_build_object(
      ...
      'note', 'Evidence-bearing no-signal result accepted. Model-reported read IDs were retained separately; canonical read IDs are the server-supplied batch IDs.'
    ),
    true
  );
...
begin
  ...
  has_signal :=
    ...
    or jsonb_array_length(case when jsonb_typeof(new.summary_json -> 'behavior_signals') = 'array' then new.summary_json -> 'behavior_signals' else '[]'::jsonb end) > 0
    or jsonb_array_length(case when jsonb_typeof(new.summary_json -> 'weak_signals') = 'array' then new.summary_json -> 'weak_signals' else '[]'::jsonb end) > 0;
  content_valid := lower(coalesce(new.summary_json ->> 'analysis_is_validated', 'false')) in ('true', '1', 'yes')
    and lower(coalesce(new.summary_json ->> 'fallback_used', 'false')) not in ('true', '1', 'yes')
    and has_evidence
    and has_signal;
  ...
  if new.status in ('queued', 'needs_review')
    ...
    new.summary_json := jsonb_set(
      ...
      jsonb_build_object(
        ...
        'missing_read_article_ids', '[]'::jsonb,
        'server_processed_article_ids', true,
        'note', 'Model UUID echo mismatch was retained for audit; canonical read IDs are the server-supplied batch IDs.'
      ),
      true
    );
    new.status := 'completed';
      ...
      and not exists (
        ...
        where b.run_id = r.id
          and (
            ...
            or (
              lower(coalesce(b.summary_json ->> 'no_signal_detected', b.summary_json ->> 'no_signal_batch', 'false')) not in ('true', '1', 'yes')
