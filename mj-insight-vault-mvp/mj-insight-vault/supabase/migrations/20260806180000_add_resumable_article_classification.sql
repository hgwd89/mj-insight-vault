-- Reclassify the complete formal corpus with a resumable, low-cost LLM worker.
-- The category gate requires this version; legacy rule-based rows remain visible but
-- cannot support a formal category-wide report.
...
values(
  ...
  'その他・分類保留',
  null,
  '既存カテゴリに十分な根拠をもって分類できない記事。無理なカテゴリ付与を避けるための明示的な保留先。',
  '{}'::text[],
  true
)
...
create table if not exists public.article_classification_jobs(
  ...
  classifier_version text not null default 'article_category_profile_v2',
...
create unique index if not exists article_classification_jobs_article_version_uidx
  on public.article_classification_jobs(article_id,classifier_version);
create index if not exists article_classification_jobs_claim_idx
  on public.article_classification_jobs(status,next_retry_at,lease_expires_at,updated_at);
...
begin
  ...
  where p_force
     or not exists(
       select 1 from public.article_profiles p
       where p.article_id=a.id and p.profile_model='article_category_profile_v2'
     )
     or not exists(
       select 1 from public.article_category_memberships m
       where m.article_id=a.id and m.source='article_category_profile_v2'
     )
  ...
  if p_force then
    ...
    where j.classifier_version='article_category_profile_v2'
      ...
      and (
        not exists(select 1 from public.article_profiles p where p.article_id=j.article_id and p.profile_model='article_category_profile_v2')
        or not exists(select 1 from public.article_category_memberships m where m.article_id=j.article_id and m.source='article_category_profile_v2')
      );
    get diagnostics v_requeued=row_count;
  end if;
  ...
  return jsonb_build_object(
    ...
    'queued_count',(select count(*) from public.article_classification_jobs where classifier_version='article_category_profile_v2' and status='queued')
  );
end;
$$;

create or replace function public.claim_article_classification_jobs_v2(
  p_limit integer default 6,
  p_lease_seconds integer default 210
)
returns setof public.article_classification_jobs
language plpgsql
security definer
set search_path=pg_catalog,public
as $$
begin
  ...
  with candidates as(
    ...
    join public.formal_corpus_articles_v1 a on a.id=j.article_id
    where j.classifier_version='article_category_profile_v2'
      and (j.next_retry_at is null or j.next_retry_at<=now())
      and (
        j.status='queued'
        or (j.status='running' and (j.lease_expires_at is null or j.lease_expires_at<=now()))
        or (j.status='failed' and j.attempt_count<3)
      )
    order by case j.status when 'running' then 0 when 'failed' then 1 else 2 end,j.updated_at,j.id
  ...
  set status='running',
      ...
      lease_expires_at=now()+make_interval(secs=>greatest(60,least(coalesce(p_lease_seconds,210),300))),
      heartbeat_at=now(),
      updated_at=now(),
      error_message=null
  from candidates c
...
declare
  ...
  v_primary text:=btrim(coalesce(p_profile->>'primary_category',''));
  v_secondary text[]:=array(select value from jsonb_array_elements_text(case when jsonb_typeof(p_profile->'secondary_categories')='array' then p_profile->'secondary_categories' else '[]'::jsonb end));
...
begin
  ...
  for v_member in select value from jsonb_array_elements(p_memberships) loop
    ...
    if v_category='' or not exists(select 1 from public.analysis_categories c where c.id=v_category and c.is_active=true) then
    ...
    values(
      ...
      v_member_score,
      v_member_confidence,
      array(select value from jsonb_array_elements_text(case when jsonb_typeof(v_member->'match_terms')='array' then v_member->'match_terms' else '[]'::jsonb end) limit 12),
  ...
  insert into public.article_profiles(
    article_id,profile_model,primary_category,secondary_categories,consumer_scene,market_signal,product_type,consumer_need,confidence,reason,profile_json,updated_at
  ) values(
    v_job.article_id,
    ...
    p_profile||jsonb_build_object('classifier_version','article_category_profile_v2','model_used',v_job.model,'classified_at',now()),
    now()
  )
  ...
  insert into public.article_category_memberships(article_id,category_id,score,confidence,source,match_terms,reason,updated_at)
  ...
  set status='completed',
      result_json=jsonb_build_object('profile',p_profile,'memberships',p_memberships),
      error_message=null,
      lease_token=null,
      lease_expires_at=null,
      heartbeat_at=now(),
...
declare
  ...
  v_retry boolean;
begin
  select * into v_job from public.article_classification_jobs j
...
create or replace view public.article_classification_status_v2
with(security_invoker=true)
as
select
  (select count(*)::integer from public.formal_corpus_articles_v1) as formal_article_count,
  count(*) filter(where j.status='queued')::integer as queued_count,
  count(*) filter(where j.status='running')::integer as running_count,
  ...
  count(*) filter(where j.status='completed' and exists(select 1 from public.article_profiles p where p.article_id=j.article_id and p.profile_model='article_category_profile_v2'))::integer as validated_profile_count,
  count(*) filter(where j.status='completed' and exists(select 1 from public.article_category_memberships m where m.article_id=j.article_id and m.source='article_category_profile_v2'))::integer as validated_membership_article_count,
  min(j.next_retry_at) filter(where j.status='queued' and j.next_retry_at>now()) as next_retry_at,
  max(j.updated_at) as last_updated_at
from public.article_classification_jobs j
where j.classifier_version='article_category_profile_v2';

revoke all on public.article_classification_status_v2 from public,anon,authenticated;
grant select on public.article_classification_status_v2 to postgres,service_role;

-- Tighten the existing category gate to the validated classifier version.
create or replace view public.category_classification_gate_v1
with(security_invoker=true)
as
with formal as(select id from public.formal_corpus_articles_v1),
profiled as(
  select distinct p.article_id from public.article_profiles p join formal f on f.id=p.article_id
  where p.profile_model='article_category_profile_v2'
),
categorized as(
...
invalid_memberships as(
  ...
  where m.source='article_category_profile_v2' and c.id is null
)
select
  ...
  case when (select count(*) from formal)=0 then 'failed' when (select count(*) from formal)<>(select count(*) from profiled) then 'failed' when (select count(*) from formal)<>(select count(*) from categorized) then 'failed' when (select n from invalid_memberships)>0 then 'failed' else 'passed' end category_classification_gate,
  case when (select count(*) from formal)=0 then 'no_formal_articles' when (select count(*) from formal)<>(select count(*) from profiled) then 'unprofiled_articles_exist' when (select count(*) from formal)<>(select count(*) from categorized) then 'uncategorized_articles_exist' when (select n from invalid_memberships)>0 then 'inactive_or_missing_category_memberships_exist' else 'passed' end gate_reason;
