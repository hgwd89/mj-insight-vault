...
begin
  ...
  into total_count, valid_count, distinct_article_count, theme_count, direct_count, supply_count
  from checked;
...
declare
  ...
  database_semantic_integrity boolean := false;
...
begin
  ...
  if run_id is not null then
    ...
    database_semantic_integrity:=public.report_hierarchical_semantic_integrity_v1(payload);
  end if;
  ...
  formal := full_corpus_candidate and metadata_integrity and database_run_integrity and database_raw_integrity and database_analytical_integrity and database_semantic_integrity and new.source_job_id is not null and valid_validation_pair and quality_status='passed' and not provisional and not fallback_used;
  ...
  if tg_op='INSERT' and full_corpus_candidate and not database_semantic_integrity then
    raise exception using errcode='23514',message='formal_report_semantic_evidence_integrity_failed',detail='Hierarchical evidence must preserve theme metadata, direct-consumer balance and semantic claim-to-article alignment.';
  end if;
  ...
  new.analysis_verification_status:=case
    ...
    when not database_semantic_integrity and full_corpus_candidate then 'semantic_evidence_unverified'
