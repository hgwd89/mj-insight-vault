...
as $function$
declare j public.ocr_verification_page_jobs_v2%rowtype;r jsonb;v_input integer;v_total integer;
begin
  ...
  if jsonb_typeof(p_rows)<>'array' or jsonb_array_length(p_rows)<1 then raise exception 'ocr_crop_v4_rows_required'; end if;
  ...
  return jsonb_build_object('status','stored','chunk_rows',v_input,'total_rows',v_total,'expected_rows',j.article_count,'complete',v_total=j.article_count);
end
$function$;
