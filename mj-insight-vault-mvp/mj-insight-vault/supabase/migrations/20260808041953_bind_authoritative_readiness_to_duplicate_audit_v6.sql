...
select
    ...
    greatest(sr.expected_article_count-sr.source_grounded_article_count,0)::integer missing_or_invalid_source_region_count,
    sr.source_grounded_page_count partition_complete_page_identity_count,
    sr.source_region_gate,
    sr.source_region_gate_reason,
    sr.job_count total_partition_jobs,
    0::integer queued_partition_jobs,
    0::integer running_partition_jobs,
    0::integer review_partition_jobs,
    sr.completed_jobs completed_partition_jobs,
    0::integer failed_partition_jobs,
    0::integer pages_needing_secondary_grounding,
    0::integer articles_needing_secondary_grounding,
    eg.strict_embedding_count,
    eg.embedding_gate,
    eg.gate_reason embedding_gate_reason,
    dg.audit_run_id duplicate_audit_run_id,
    coalesce(dg.candidate_pair_count,0) duplicate_candidate_pair_count,
    coalesce(dg.reviewed_distinct_pair_count,0) reviewed_distinct_pair_count,
    coalesce(dg.reviewed_duplicate_pair_count,0) reviewed_duplicate_pair_count,
    coalesce(dg.unresolved_pair_count,0) unresolved_pair_count,
    dg.duplicate_gate,
    dg.gate_reason duplicate_gate_reason,
    ...
    inv.inventory_gate,
    inv.pages inventory_pages,
    inv.jobs inventory_jobs,
    inv.completed completed_inventory_jobs,
    inv.discovery_required inventory_discovery_required,
    inv.needs_review inventory_needs_review,
    og.ocr_verification_gate ocr_readiness_gate,
    ov.strong_region_count,
    ov.review_region_count,
    ov.low_region_count,
    og.verified::bigint verified_article_count
