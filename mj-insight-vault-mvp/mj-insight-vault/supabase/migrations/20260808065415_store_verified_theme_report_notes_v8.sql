...
as $function$
declare j public.verified_theme_report_note_jobs_v8%rowtype;g public.verified_theme_report_note_passes_v8%rowtype;c public.verified_theme_report_note_passes_v8%rowtype;v_ids uuid[];v_support integer;v_final uuid;
begin
  ...
  if g.job_id is null or c.job_id is null or g.model=c.model or g.provider_response_id=c.provider_response_id or g.prompt_sha256=c.prompt_sha256 then raise exception 'verified_report_note_v8_independent_passes_required'; end if;
  if not coalesce((c.result_json->>'approved')::boolean,false) or not coalesce((c.result_json->>'evidence_supported')::boolean,false) or not coalesce((c.result_json->>'trend_consistent')::boolean,false) or not coalesce((c.result_json->>'limitation_adequate')::boolean,false) or coalesce((c.result_json->>'overclaim_risk')::boolean,true) then
    update public.verified_theme_report_note_jobs_v8 set status='needs_review',active_pass_kind=null,lease_token=null,lease_expires_at=null,error_message=left(coalesce(c.result_json->>'reason','report note critic did not approve'),2000),updated_at=now() where id=j.id;
  ...
  select support_article_count into v_support from public.verified_theme_metrics_v8 where candidate_id=j.candidate_id;
  if v_support>0 and cardinality(v_ids)=0 then raise exception 'verified_report_note_v8_evidence_required'; end if;
  if exists(select 1 from unnest(v_ids) x where not exists(select 1 from public.verified_theme_deterministic_evidence_v8 e where e.candidate_id=j.candidate_id and e.article_id=x)) then raise exception 'verified_report_note_v8_evidence_id_invalid'; end if;
  ...
  return jsonb_build_object('status','completed','final_job_id',v_final);
end
$function$;

create or replace function public.store_verified_theme_report_note_pass_v8(p_job_id uuid,p_lease_token uuid,p_pass_kind text,p_model text,p_provider_response_id text,p_prompt_sha256 text,p_response_sha256 text,p_result jsonb)
returns jsonb
language plpgsql security definer set search_path=pg_catalog,public
as $function$
...
begin
  select * into j from public.verified_theme_report_note_jobs_v8 where id=p_job_id for update;
  if not found or j.status<>'running' or j.lease_token is distinct from p_lease_token or j.lease_expires_at<now() then raise exception 'verified_report_note_v8_lease_invalid'; end if;
  if p_pass_kind not in ('generator','critic') or j.active_pass_kind is distinct from p_pass_kind then raise exception 'verified_report_note_v8_pass_invalid'; end if;
  if coalesce(btrim(p_model),'')='' or coalesce(btrim(p_provider_response_id),'')='' or p_prompt_sha256 !~ '^[0-9a-f]{64}$' or p_response_sha256 !~ '^[0-9a-f]{64}$' or jsonb_typeof(p_result)<>'object' then raise exception 'verified_report_note_v8_receipt_or_result_invalid'; end if;
  select * into o from public.verified_theme_report_note_passes_v8 where job_id=j.id and pass_kind<>p_pass_kind;
  if found and (o.model=p_model or o.provider_response_id=p_provider_response_id or o.prompt_sha256=p_prompt_sha256) then raise exception 'verified_report_note_v8_independent_pass_required'; end if;
  if p_pass_kind='generator' then
    if char_length(btrim(coalesce(p_result->>'interpretation','')))<8 or char_length(btrim(coalesce(p_result->>'trajectory_interpretation','')))<4 or char_length(btrim(coalesce(p_result->>'limitation','')))<4 or jsonb_typeof(p_result->'evidence_article_ids')<>'array' then raise exception 'verified_report_note_v8_generator_schema_invalid'; end if;
    if (p_result->>'interpretation') ~ '[0-9０-９]' or (p_result->>'trajectory_interpretation') ~ '[0-9０-９]' or (p_result->>'limitation') ~ '[0-9０-９]' then raise exception 'verified_report_note_v8_freeform_numbers_forbidden'; end if;
    ...
    if v_ids<>v_distinct then raise exception 'verified_report_note_v8_duplicate_evidence_ids'; end if;
    if exists(select 1 from jsonb_array_elements_text(p_result->'evidence_article_ids') x where x !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' or not exists(select 1 from public.verified_theme_deterministic_evidence_v8 e where e.candidate_id=j.candidate_id and e.article_id=x::uuid)) then raise exception 'verified_report_note_v8_evidence_id_invalid'; end if;
    delete from public.verified_theme_report_note_passes_v8 where job_id=j.id and pass_kind='critic';
  else
    if not exists(select 1 from public.verified_theme_report_note_passes_v8 where job_id=j.id and pass_kind='generator') then raise exception 'verified_report_note_v8_critic_requires_generator'; end if;
    if jsonb_typeof(p_result->'approved')<>'boolean' or jsonb_typeof(p_result->'evidence_supported')<>'boolean' or jsonb_typeof(p_result->'trend_consistent')<>'boolean' or jsonb_typeof(p_result->'limitation_adequate')<>'boolean' or jsonb_typeof(p_result->'overclaim_risk')<>'boolean' or char_length(btrim(coalesce(p_result->>'reason','')))<4 then raise exception 'verified_report_note_v8_critic_schema_invalid'; end if;
  end if;
