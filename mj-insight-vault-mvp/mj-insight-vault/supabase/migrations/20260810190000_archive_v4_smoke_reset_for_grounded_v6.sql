-- Archive the V4 visual-inventory smoke state, reset the recovered inventory jobs,
...
-- Retire the two V4 smoke pages that reached materialization. They are archived above.
...
-- Keep requires_third_pass conservative: V4 smoke may have raised it, which only adds an
-- independent adjudicator call. Do not recompute via heavy OCR views during migration.
