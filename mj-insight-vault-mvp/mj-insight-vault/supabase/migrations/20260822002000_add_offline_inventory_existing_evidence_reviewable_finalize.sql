create or replace function public.finalize_inventory_offline_queued_existing_evidence_reviewable_v1(
  p_job_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  if coalesce(v_enabled,true) or coalesce(v_third,true) then
    raise exception 'offline_existing_evidence_reviewable_requires_execution_disabled';
  end if;
  ...
  if v_freeze is null
    ...
    raise exception 'offline_existing_evidence_reviewable_job_not_eligible';
  end if;

  if exists(select 1 from public.source_region_materialization_receipts_v6 r where r.inventory_job_id=j.id) then
    raise exception 'offline_existing_evidence_reviewable_materialized_job';
  end if;
  ...
  if v_passes<>3
     ...
     or v_responses<>3
     or v_prompts<>3 then
    raise exception 'offline_existing_evidence_reviewable_three_independent_passes_required';
  end if;

  v_consensus_source := public.inventory_consensus_source_v3(j.id);
  if v_consensus_source is null then
    raise exception 'offline_existing_evidence_reviewable_consensus_missing';
  end if;
  ...
  if v_groups<>j.existing_article_count then
    raise exception 'offline_existing_evidence_reviewable_article_count_mismatch:%/%',
  ...
  if jsonb_array_length(public.inventory_structural_article_group_violations_v1(j.id))<>0 then
    raise exception 'offline_existing_evidence_reviewable_structural_violation';
  end if;

  update public.inventory_v3_execution_control_v1
     set enabled=true,
         grounded_third_pass_enabled=true,
         reason='transaction-local reviewable finalize of queued existing three-pass evidence; no API',
         updated_at=now()
   where singleton=true;
  ...
  if c.id is null then
    raise exception 'offline_existing_evidence_reviewable_claim_failed';
  end if;
