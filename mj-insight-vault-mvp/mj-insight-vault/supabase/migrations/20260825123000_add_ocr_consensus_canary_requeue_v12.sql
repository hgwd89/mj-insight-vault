...
create table if not exists public.ocr_consensus_requeue_archives_v12 (
...
alter table public.ocr_consensus_requeue_archives_v12 enable row level security;
revoke all on table public.ocr_consensus_requeue_archives_v12 from public, anon, authenticated;
grant select, insert on table public.ocr_consensus_requeue_archives_v12 to postgres, service_role;
...
begin
  ...
  if not found then
    raise exception 'ocr_consensus_v12_job_missing';
  end if;
  if j.is_canary is distinct from true then
    raise exception 'ocr_consensus_v12_canary_only';
  end if;
  if j.status = 'running' and j.lease_expires_at is not null and j.lease_expires_at > now() then
    raise exception 'ocr_consensus_v12_active_lease';
  end if;
  if coalesce(nullif(btrim(p_reason), ''), '') = '' then
    raise exception 'ocr_consensus_v12_reason_required';
  end if;
  ...
  select jsonb_build_object(
    ...
    'decisions', coalesce((select jsonb_agg(to_jsonb(x) order by x.created_at, x.article_id) from public.ocr_consensus_decisions_v11 x where x.job_id = j.id), '[]'::jsonb),
  ...
  return jsonb_build_object(
    ...
    'job_id', j.id,
    'status', 'queued',
    'deleted_pass_runs', v_pass_runs,
    'deleted_transcriptions', v_transcriptions,
    'deleted_decisions', v_decisions,
    'deleted_canonicals', v_canonicals
  );
end
$function$;
