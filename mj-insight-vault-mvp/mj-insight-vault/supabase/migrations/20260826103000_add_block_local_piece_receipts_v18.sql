-- v18: enrich isolated OCR receipts with block-local reading-piece provenance.
-- Existing v11 decision thresholds remain untouched.

alter table public.ocr_independent_segment_receipts_v16
  ...
  add column if not exists source_left integer,
  add column if not exists source_top integer,
  add column if not exists source_right integer,
  add column if not exists source_bottom integer;
...
begin
  ...
  if p_piece_kind not in ('whole_block','vertical_segment') then raise exception 'ocr_consensus_v18_piece_kind_invalid'; end if;
  ...
  return jsonb_build_object('status','stored','article_id',p_article_id,'pass_kind',p_pass_kind,'sequence',p_sequence,'segment_count',p_segment_count,'block_index',p_block_index,'piece_sequence',p_piece_sequence,'piece_count',p_piece_count);
end
$function$;
