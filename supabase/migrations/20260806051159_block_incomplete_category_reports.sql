...
select
  ...
  case when (select count(*) from formal)=0 then 'failed' when (select count(*) from formal)<>(select count(*) from profiled) then 'failed' when (select count(*) from formal)<>(select count(*) from categorized) then 'failed' when (select n from invalid_memberships)>0 then 'failed' else 'passed' end as category_classification_gate,
  case when (select count(*) from formal)=0 then 'no_formal_articles' when (select count(*) from formal)<>(select count(*) from profiled) then 'unprofiled_articles_exist' when (select count(*) from formal)<>(select count(*) from categorized) then 'uncategorized_articles_exist' when (select n from invalid_memberships)>0 then 'inactive_or_missing_category_memberships_exist' else 'passed' end as gate_reason;
...
as
with current_all as (
  select count(*)::integer as current_article_count from public.formal_corpus_articles_v1
),
current_category as (
...
classification as (
  select category_classification_gate, gate_reason from public.category_classification_gate_v1
)
select r.id,r.scope_type,r.scope_query,r.status,r.model,r.active_article_count,
  case when r.scope_type='all' then ca.current_article_count when r.scope_type='category' then coalesce(cc.current_article_count,0) else r.active_article_count end as current_article_count,
  case when r.scope_type='all' then ca.current_article_count when r.scope_type='category' then coalesce(cc.current_article_count,0) else r.active_article_count end-r.active_article_count as current_article_count_diff,
...
begin
  if target_scope<>'category' or category_id='' or corpus_gate<>'passed' or generation_status_value='blocked' or report_kind_value='diagnostic' or report_chat then return new; end if;
  select exists(select 1 from public.analysis_categories c where c.id=category_id and c.is_active=true) into category_exists;
  if not category_exists then raise exception using errcode='23514', message='formal_category_id_invalid', detail='Formal category reports require an active analysis category.'; end if;
  select category_classification_gate,gate_reason into classification_gate,classification_reason from public.category_classification_gate_v1;
  if classification_gate<>'passed' then raise exception using errcode='23514', message='formal_category_classification_incomplete', detail=classification_reason; end if;
  return new;
end;
$$;

drop trigger if exists trg_00_enforce_category_report_classification_v1 on public.chat_reports;
create trigger trg_00_enforce_category_report_classification_v1 before insert or update of answer_json on public.chat_reports for each row execute function public.enforce_category_report_classification_v1();
