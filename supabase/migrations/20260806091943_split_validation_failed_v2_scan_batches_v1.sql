...
begin
  ...
  for v_batch in
    ...
    where b.run_id=p_run_id
      ...
      and (
        coalesce(b.error_message,'') like 'read_article_ids missing %'
        or coalesce(b.error_message,'') like '%consumer_narratives / behavior_signals / weak_signals are all empty%'
      )
  ...
  set total_batches=(select count(*) from public.full_corpus_scan_batches b where b.run_id=r.id),
      ...
      coverage_json=coalesce(r.coverage_json,'{}'::jsonb) || jsonb_build_object(
        'split_recovery_version','split_validation_failed_v2_scan_batches_v1',
        'split_target_size',v_target,
...
revoke all on function public.split_validation_failed_v2_scan_batches_v1(uuid,integer) from public,anon,authenticated;
grant execute on function public.split_validation_failed_v2_scan_batches_v1(uuid,integer) to service_role;