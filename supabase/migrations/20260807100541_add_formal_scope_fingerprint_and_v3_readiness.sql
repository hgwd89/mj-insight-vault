create or replace function public.formal_corpus_scope_proof_v1(
...
as $function$
  with scoped as (
    ...
    select count(*)::bigint as n,
           coalesce(string_agg(id::text || ':' || analysis_text_sha256, '|' order by id::text),'') as payload
    from scoped
  )
  select n,
...
revoke all on function public.formal_corpus_scope_proof_v1(text,text) from public,anon,authenticated;
grant execute on function public.formal_corpus_scope_proof_v1(text,text) to postgres,service_role;

create or replace view public.aaaa_pipeline_readiness_v1
...
select fs.formal_article_count,
       ...
       coalesce(vg.current_article_count,fs.formal_article_count::integer) as current_article_count,
       coalesce(vg.current_article_count_diff,0) as current_article_count_diff,
