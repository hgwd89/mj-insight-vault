with recursive edges as (
  ...
  where length(a.headline)>=8
    ...
    and similarity(a.headline,b.headline)>=0.70
  ...
  select u.cluster_id,a.id,a.created_at,a.headline,a.reconstruction_confidence,
  ...
  group by u.cluster_id,a.id,a.created_at,a.headline,a.reconstruction_confidence
), ranked as (
...
set duplicate_of_article_id=m.canonical_id,
    exclusion_reason='high_confidence_duplicate_same_source_ocr_v2',
    updated_at=now()
from mapping m
...
as
select
  id,batch_id,source_image_id,headline,article_date,article_index,ocr_text,article_type,
  has_table,has_chart,has_image,status,manual_analysis,created_at,updated_at,
  duplicate_of_article_id,exclusion_reason,enrichment_status,enrichment_error,
  analysis_text_origin,source_ocr_sha256,analysis_text_sha256,reconstruction_model,
  reconstruction_prompt_version,reconstruction_confidence,provenance_status,provenance_json
from public.articles a
where (status is null or status<>all(array['deleted'::text,'excluded'::text,'rejected'::text]))
  and coalesce(article_type,'')='article'
  and coalesce(btrim(ocr_text),'')<>''
  and source_image_id is not null
  and provenance_status=any(array['traceable'::text,'legacy_traceable'::text])
  and coalesce(source_ocr_sha256,'') ~ '^[0-9a-f]{64}$'
  and coalesce(analysis_text_sha256,'') ~ '^[0-9a-f]{64}$'
  and analysis_text_origin<>'pending'
