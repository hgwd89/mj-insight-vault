with recursive x as (
  ...
  where similarity(public.normalize_article_headline_v1(a.headline),public.normalize_article_headline_v1(b.headline))>=0.55
), adj as (
  ...
  update public.articles a
     set status='excluded',duplicate_of_article_id=r.canonical_id,
         exclusion_reason='duplicate_reviewed_same_publication_date_near_headline_v1',updated_at=now()
    from ranked r
