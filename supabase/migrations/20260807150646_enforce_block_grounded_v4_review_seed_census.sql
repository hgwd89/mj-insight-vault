...
begin
  ...
  if not public.source_region_anchor_unique_block_v4(new.article_id,v_region.source_region_id,new.source_anchor) then
    raise exception using errcode='23514',message='theme_seed_source_anchor_not_unique_in_source_block';
  end if;
  return new;
end $$;
...
begin
  ...
  if new.relation='none' then
    ...
    if not public.source_region_anchor_unique_block_v4(new.article_id,v_region.source_region_id,new.source_region_anchor) then
      raise exception using errcode='23514',message='census_source_anchor_not_unique_in_source_block';
    end if;
  end if;
  return new;
end $$;
...
with target as (
  ...
  select g.article_id,g.analysis_body_sha256,g.source_region_id,g.source_region_sha256,g.current_source_raw_ocr_sha256
  from proof p
  ...
  select r.* from public.full_corpus_article_reviews_v4 r join expected e on e.article_id=r.article_id where r.run_id=p_run_id
), seeds as (
  ...
  select s.id,
         ...
         and public.source_region_anchor_unique_block_v4(s.article_id,e.source_region_id,s.source_anchor) valid
  ...
  where p.status='completed'
    ...
    and not exists(
      ...
      where r.batch_id<>a.batch_id or r.batch_index<>a.batch_index
         ...
         or not public.article_review_anchor_integrity_v4(r.id)
    )
...
with ar as (
  ...
  select r.article_id,r.source_clean_body_sha256,r.source_region_sha256,r.source_region_id
  ...
  select r.*,
    ...
    and (
      ...
      (r.relation<>'none'
       ...
       and public.source_region_anchor_unique_block_v4(r.article_id,r.current_source_region_id,r.source_region_anchor))
    ) valid
  from relations r
), seed_counts as (
