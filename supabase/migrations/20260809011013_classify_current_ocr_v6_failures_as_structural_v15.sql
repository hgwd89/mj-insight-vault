...
begin
  ...
  if not found or j.status<>'running' or j.lease_token is distinct from p_lease_token then raise exception 'ocr_verification_v6_lease_invalid'; end if;
  v_structural:=v_message ~* '(ocr_crop_v[46]_|ocr_vision_v[46]_|ocr_verification_v[256]_|ocr_source_provenance_v[56]_|row_count|unknown or duplicate article|unknown_article|empty_transcription|low_confidence|article_set|not bijective|independent_pass|consensus|numeric_mismatch|proper_noun|crop image fingerprint|source binding|source image path or dimensions are missing|image dimension mismatch)';
...
begin
  ...
  limit 1;
  if position('ocr_crop_v[46]_' in coalesce(v_def,''))=0 or position('ocr_source_provenance_v[56]_' in coalesce(v_def,''))=0 then
    raise exception 'ocr_v6_structural_failure_classifier_not_installed';
  end if;
end
$$;
